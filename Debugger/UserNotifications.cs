...
namespace ModTools
{
    public static class UserNotifications
    {
        ...
        private static readonly string LoggingChangeNotification = @"You are using the new ModTools console.
...
Instead you should use the built-in Unity Debug API (http://docs.unity3d.com/ScriptReference/Debug.html). Example: Debug.Log(""Hello world!"");";
        ...
        private static readonly string UnityLoggingHookNotification = @"Your version of ModTools has a new feature which allows it to hook Unity's Debug logging so you can safely log from the simulation thread (or any other thread).
